...
type PromotionScheduleStatus struct {
	...
	// NextClose is when the schedule is next expected to forbid promotions, and is meaningful only while Closed is false. It allows a client to give warning of an approaching freeze. Like NextOpen it is optional, and absent when no such boundary is known.  +optional
	...
	// Reason explains in human-readable terms why the schedule is closed, naming the window responsible where one is. It is always set while Closed is true, and is the only field guaranteed to explain a freeze -- NextOpen may be absent. It carries the same explanation as the corresponding admission rejection.  +optional
